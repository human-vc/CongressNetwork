...
The recovery ratio documents the asymmetry but does not explain it. A natural hypothesis is that the two electoral waves injected qualitatively different freshman cohorts. The 104th Congress seated 96 freshmen (79 Republicans, 17 Democrats); the 112th seated 104 (88 Republicans, 16 Democrats). Similar in size and partisan composition. But the two cohorts differed sharply in their cooperative behavior.

Figure~\ref{fig:freshman_cohorts} compares the distribution of cross-party agreement rates for incoming freshmen in the 104th and 112th Congresses. In raw terms, the difference is stark. Contract freshmen had a mean cross-party agreement rate of 0.399 (IQR: $[0.376, 0.417]$), with 41\% exceeding 0.40 and 8\% exceeding 0.45. The 39 members above 0.40 are distributed across the range (31 between 0.40--0.45, 8 between 0.45--0.50), not driven by a few outliers. Tea Party freshmen had a mean of 0.318 (IQR: $[0.276, 0.333]$), with only 9\% above 0.40. A two-sample Kolmogorov-Smirnov test rejects the null of equal distributions ($D = 0.83$, $p < 10^{-34}$).

However, comparing raw rates across eras conflates individual behavior with the cooperation environment. The 112th Congress was already far more polarized than the 104th, so Tea Party freshmen faced fewer opportunities for cross-party agreement regardless of their own preferences. To disentangle composition from environment, we normalize each freshman's cross-party agreement rate by subtracting the incumbent mean for their Congress (0.402 for the 104th, 0.306 for the 112th). Era-adjusted, the two cohorts are indistinguishable: 38.5\% of Contract freshmen and 42.3\% of Tea Party freshmen exceeded their era's incumbent baseline (KS $D = 0.093$, $p = 0.74$). Both classes cooperated at roughly the rate their chamber's norms permitted.

This null result on era-adjusted rates is the key finding. It means the mechanism is compositional rather than individual: the Tea Party freshmen were not unusually uncooperative for their era; the era itself had already eliminated the conditions for cross-party cooperation. The Contract wave entered a chamber where cooperating across the aisle was still structurally possible, so freshmen who matched the chamber baseline automatically contributed to bipartisan connectivity. The Tea Party wave entered a chamber where the baseline was near zero, so even freshmen at the norm contributed nothing to cross-party bridging. The recovery asymmetry is not about the character of the incoming class but about the structural environment that received them. Once the cooperation environment collapsed, subsequent cohorts, no matter how individually moderate, could not rebuild it.
